-- Query to extracts observations of "Parc National des Calanques" for PACA with sensitive data !
-- Usage (from local computer): cat ./pnc_observations.sql | ssh <user>@<ip-server> 'export PGPASSWORD="<db-user-password>" ; psql -h localhost -p <db-port> -U <db-user> -d <db-name>' > ./$(date +'%F')_observations_pnc.csv
...
COPY (
    ...
    WHERE o.uuid_organisme = '637264ef-fcb2-3b49-e053-2614a8c02066' -- Parc national des Calanques
) TO stdout
