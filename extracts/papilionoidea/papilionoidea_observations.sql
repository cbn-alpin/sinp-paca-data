-- Query to extracts Papilionoidea observations for PACA with sensitive data !
-- Usage (from local computer): cat ./papilionoidea_observations.sql | ssh <user>@<ip-server> 'export PGPASSWORD="<db-user-password>" ; psql -h localhost -p <db-port> -U <db-user> -d <db-name>' > ./$(date +'%F')_observations_papilionoidea.csv
