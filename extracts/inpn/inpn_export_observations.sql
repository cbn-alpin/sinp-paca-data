...
-- Usage (from local computer): cat ./inpn_export_observations.sql | ssh <user>@<ip-server> 'export PGPASSWORD="<db-user-password>" ; psql --no-psqlrc -h localhost -p <db-port> -U <db-user> -d <db-name>' > ./$(date +'%F')_inpn_obs_extracts.csv
...
COPY (
    WITH cda AS (
        ...
        FROM
            gn_meta.t_datasets AS d
            JOIN gn_meta.cor_dataset_actor AS act ON act.id_dataset = d.id_dataset
            JOIN utilisateurs.bib_organismes AS orga ON orga.id_organisme = act.id_organism
        GROUP BY
            d.id_dataset
    ),
    mailles AS (
        SELECT
            s.id_synthese,
            a.geom AS geom_maille
        FROM
            gn_synthese.synthese AS s
            JOIN ref_geo.l_areas AS a ON st_within(st_transform(s.the_geom_point, 2154), a.geom)
            JOIN ref_geo.bib_areas_types AS bat ON bat.id_type = a.id_type
        WHERE
            bat.type_code = 'M5'
            AND s.the_geom_point IS NOT NULL
    ),
    --Faune, floutage à la maille 5 km ; récupération de la maille du centroïde
    faune AS (
        SELECT
            s.id_synthese,
            st_asewkt(m.geom_maille) AS geometrie,
            NULL AS nomLieu,
        ...
        FROM
            ...
            JOIN mailles AS m ON s.id_synthese = m.id_synthese
        WHERE
            t.regne = 'Animalia'
    ),
    --Flore Pr/NSP floutage à la maille 5 km ; récupération de la maille du centroïde
    flore_pr AS (
        SELECT
            s.id_synthese,
            st_asewkt(m.geom_maille) AS geometrie,
        ...
        FROM
            gn_synthese.synthese AS s
            JOIN taxonomie.taxref AS t ON s.cd_nom = t.cd_nom
            JOIN gn_meta.t_datasets AS td ON s.id_dataset = td.id_dataset
            JOIN mailles AS m ON s.id_synthese = m.id_synthese
        WHERE
            t.regne IN ('Plantae', 'Fungi', 'Protozoa')
    ...
    FROM
        observations AS gl
        JOIN gn_synthese.synthese AS s ON gl.id_synthese = s.id_synthese
        JOIN taxonomie.taxref AS t ON t.cd_nom = s.cd_nom
        JOIN gn_meta.t_datasets AS d ON d.id_dataset = s.id_dataset
        JOIN gn_meta.t_acquisition_frameworks AS af ON d.id_acquisition_framework = af.id_acquisition_framework
        JOIN gn_synthese.t_sources AS sources ON sources.id_source = s.id_source
        ...
        LEFT JOIN ref_nomenclatures.t_nomenclatures AS n21 ON d.id_nomenclature_data_origin = n21.id_nomenclature
