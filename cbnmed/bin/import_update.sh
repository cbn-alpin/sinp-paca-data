#!/bin/bash
# Encoding : UTF-8
# Import some updated, deleted or new data in GeoNature Database the CBNMED Flora Data.
...
function main() {
    ...
    redirectOutput "${cbnmed_log_imports}"
    checkSuperuser
...
function downloadCenpacaDataArchive() {
    ...
    if [[ ! -f "${raw_dir}/${cbnmed_filename_archive}" ]]; then
        curl -X POST https://content.dropboxapi.com/2/files/download \
            --header "Authorization: Bearer ${cbnmed_dropbox_token}" \
            --header "Dropbox-API-Arg: {\"path\": \"${cbnmed_dropbox_dir}/${cbnmed_filename_archive}\"}" \
            > "${raw_dir}/${cbnmed_filename_archive}"
     else
        printVerbose "Archive file \"${cbnmed_filename_archive}\" already downloaded." ${Gra}
    fi
}
...
function extractArchive() {
    ...
    if [[ -f "${raw_dir}/${cbnmed_filename_archive}" ]]; then
        if [[ ! -f "${raw_dir}/${cbnmed_filename_synthese}" ]]; then
            cd "${raw_dir}/"
            tar jxvf "${raw_dir}/${cbnmed_filename_archive}"
        else
...
function buildTablePrefix() {
    table_prefix="${app_code}_${cbnmed_import_date//-/}"
}
...
function parseCsv() {
    ...
    declare -n csv_file="cbnmed_filename_${data_type_abbr}"
