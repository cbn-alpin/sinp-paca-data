...
INSERT INTO t_datasets (
    ...
    marine_domain,
    terrestrial_domain,
    -- id_nomenclature_dataset_objectif, -- Not include to set default value
    bbox_west,
    bbox_east,
    bbox_south,
    bbox_north,
    -- id_nomenclature_collecting_method, -- Not include to set default value
    id_nomenclature_data_origin,
    id_nomenclature_source_status,
    meta_create_date,
    meta_update_date
...
SELECT
    ...
    terrestrial_domain,
    -- id_nomenclature_dataset_objectif, -- Not include to set default value
    -- Transform bbox coordinates from SRID 2154 to 4326
	ST_X(ST_Transform(ST_SetSRID(ST_MakePoint(bbox_west, bbox_north), 2154), 4326)) AS bbox_west,
	ST_X(ST_Transform(ST_SetSRID(ST_MakePoint(bbox_east, bbox_south), 2154), 4326)) AS bbox_east,
	ST_Y(ST_Transform(ST_SetSRID(ST_MakePoint(bbox_east, bbox_south), 2154), 4326)) AS bbox_south,
	ST_Y(ST_Transform(ST_SetSRID(ST_MakePoint(bbox_west, bbox_north), 2154), 4326)) AS bbox_north,
    -- id_nomenclature_collecting_method, -- Not include to set default value
    id_nomenclature_data_origin,
    id_nomenclature_source_status,
    meta_create_date,
