...
-- gn_synthese.v_synthese_for_export source
...
CREATE OR REPLACE VIEW gn_synthese.v_synthese_for_export AS
    ...
    FROM gn_synthese.synthese AS s
        ...
        LEFT JOIN LATERAL (
            SELECT
                cas.id_synthese,
                string_agg(DISTINCT concat(a_1.area_name, ' (', a_1.area_code, ')'), ', ') AS communes
        ...
        LEFT JOIN LATERAL (
            SELECT
                ...
                string_agg(DISTINCT bo.nom_organisme, ', ') AS organisms
            FROM gn_meta.t_datasets AS td
                LEFT JOIN gn_meta.cor_dataset_actor AS cad
                    ON cad.id_dataset = td.id_dataset AND (
                        cad.id_nomenclature_actor_role =
